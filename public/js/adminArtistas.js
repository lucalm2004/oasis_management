...
function listarArtistas(valor) {
    ...
    ajax.open('POST', 'admin7/crudartistas');
...
function Eliminar(id) {
    ...
    Swal.fire({
        ...
        text: `¿Seguro que desea eliminar el artista?`,
        icon: "warning",
            ...
            fetch(`admin7/crudartistas/${id}`, {
                ...
                .then(data => {
                    ...
                    if (data.success == true) {
                        Swal.fire({
                            title: "Borrado",
                            text: "Artista eliminado correctamente",
                            icon: "success"
                        }).then(() => {
                            listarArtistas('');
...
function Editar(id) {
    ...
    fetch(`admin7/crudartistas/modadmin/${id}`)
        .then(response => response.json())
        .then(data => {
            // Crear el formulario con los datos recuperados
            Swal.fire({
                title: "Modificar Artista",
...
/* Crear usurio */
var crearArtista = document.getElementById("crearArtista");
crearArtista.addEventListener("click", function() {
    mostrarFormulario();
...
function enviarFormulario() {
    ...
    fetch('admin7/crudartistas/insertartista', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error('Error al crear el artista: ' + response.statusText);
            }
            return response.json();
        })
        .then(data => {
            ...
            Swal.fire({
                title: "Creado",
                text: "Artista creado correctamente",
                icon: "success"
            }).then(() => {
                listarArtistas('');
        ...
        .catch(error => {
            console.error('Error:', error);
            Swal.fire('Error', 'Error al crear el artista', 'error');
        });
}
