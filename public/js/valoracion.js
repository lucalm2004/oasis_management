  ...
  let eventosVisible = false; // Variable de estado para controlar la visibilidad de la sección de eventos
  let reviewsVisible = false; // Variable de estado para controlar la visibilidad de la sección de reseñas

  function showEventsAndRate(idDiscoteca) {
      const eventsContainer = document.getElementById('eventos-container');

      if (eventosVisible) {
          eventsContainer.innerHTML = ''; // Limpiamos el contenido
          eventosVisible = false; // Actualizamos el estado
      ...
      fetch(`/discotecas/${idDiscoteca}/eventos`)
          ...
          .then(data => {
              ...
              eventosVisible = true; // Actualizamos el estado
          })
          .catch(error => {
          ...
          <div class="card mb-3 custom-card">
              <div class="card-body">
                  ...
                  <button onclick="submitRating(${evento.id}, ${evento.id_discoteca})"
                      class="btn btn-primary custom-btn">Valorar</button>
              </div>
          </div>
      `;
          ...
          eventsContainer.innerHTML = '<p style="color: white;">No hay eventos disponibles en este momento.</p>';
      }
  }
