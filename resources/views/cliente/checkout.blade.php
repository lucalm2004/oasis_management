...
<div class="iphone">
    ...
    <form action="/cliente/checkout/pay" class="form" method="GET">
      <div>
...
document.getElementById('precioTotal').value = totalConEnvio.toFixed(2);
    var count = 0; // Inicializar el contador
    var totalEntradas = 0; // Inicializar el total de entradas

    if (Array.isArray(carrito)) { // Verificar si carrito es un arreglo
        totalEntradas = carrito.length; // Contar la cantidad de elementos en el arreglo
    ...
    var carritoHTML = '';
    carritoHTML += '<?php $user = Auth::user(); echo "Nombre: <b> $user->name</b> "; echo "<br /> Correo: "; echo "<b>$user->email</b>"; ?></br>';
console.log(carrito)
carrito.forEach(function(item) {
  document.getElementById('nombreEvento').value = item.name;
    if (item.precio_total === '10') {
        carritoHTML += 'Entrada: <b>' + item.name + ' - 1 Cons.</b><br>';
        console.log('1');

    } else if (item.precio_total === '15') {
        carritoHTML += 'Entrada: <b>' + item.name + ' - 2 Cons.</b><br>';
        console.log('2');

    } else {
        carritoHTML += 'Entrada: <b>' + item.name + ' - Entrada VIP</b><br>';
        console.log('3');
    }
});

    carritoHTML += 'Total de entradas: <b>' + totalEntradas + ' entradas</b><br>';
