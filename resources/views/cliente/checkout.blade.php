...
<head>
    ...
    <style>
        :root {
            --color-background: #10102a;
            --color-primary: #F5763B;
...
<body>
    ...
    <div class="iphone">
        ...
        <form action="{{ route('checkout') }}" class="form" method="GET">
            <div>
                <h2>Datos de la compra:</h2>

                <div class="card">
                    <address id="carritoInfo">
                        <?php
                        $user = Auth::user();
            ...
            <div>
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            <td>Price Total</td>
                            <td align="right" id="precio">$84.82</td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td>Total</td>
                            <td align="right" id="total">$88.36</td>
                            <input type="hidden" id="precioTotal" name='precioTotal' value="92">
                            <input type="hidden" id="nombreEvento" name="nombreEvento" value="a">
                            <input type="hidden" name="totalEntradas" id="totalEntradas" value="a">
                            <input type="hidden" name="tipoEntrada" id="tipoEntrada" value="a">
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div>
                <button id="btnSuccess" class="button button--full" type="submit"><svg class="icon">
                        <use xlink:href="#icon-shopping-bag" />
                    </svg>Buy Now</button>
                <input type="hidden" name="discoteca" value="<?php echo $_GET['discoteca']; ?>">
            </div>
        </form>
    </div>

    <div id="nombreCancion" style="display: none;">{{ $nombreCancion }}</div>
...
<script>
    // Reiniciar variables en localStorage
    ...
    window.onload = function() {
        obtenerCancionesSeleccionadas();
        obtenerBonificacionesCanjeadas();
    };

    // Obtener variables reiniciadas
    var carrito = JSON.parse(localStorage.getItem('carrito')) || [];
    var precioTotal = parseFloat(localStorage.getItem('precioTotal')) || 0;

    // Mostrar precio total
    document.getElementById('precio').innerHTML = precioTotal.toFixed(2) + " €";
    var totalConEnvio = precioTotal + 2.99;
    document.getElementById('total').innerHTML = totalConEnvio.toFixed(2) + " €";
    document.getElementById('precioTotal').value = totalConEnvio.toFixed(2);

    // Contar el total de entradas
    var totalEntradas = 0;
    if (Array.isArray(carrito)) {
        totalEntradas = carrito.length;
        document.getElementById('totalEntradas').value = totalEntradas;
    } else {
        console.log('El contenido de carrito no es un arreglo válido.');
    }

    // Construir la información del carrito
    var carritoHTML = '';
    carritoHTML += "Nombre: <b><?php echo $user->name; ?></b><br>";
    carritoHTML += "Correo: <b><?php echo $user->email; ?></b><br>";
    var tipoEntrada;
    ...
    carrito.forEach(function(item) {
        ...
        document.getElementById('nombreEvento').value = item.nombre_evento;
        if (item.precio_total === '10') {
            carritoHTML += 'Entrada: <b>' + item.nombre_evento + ' - 1 Cons.</b><br>';
            tipoEntrada = 0;
            console.log(tipoEntrada);

            console.log('1');
        } else if (item.precio_total === '15') {
            carritoHTML += 'Entrada: <b>' + item.nombre_evento + ' - 2 Cons.</b><br>';
            console.log('2');
            tipoEntrada = 0;
            console.log(tipoEntrada);

        } else if (item.precio_total === '125') {
            carritoHTML += 'Entrada: <b>' + item.nombre_evento + ' - Entrada VIP</b><br>';
            console.log('3');
            tipoEntrada = 1;
            console.log(tipoEntrada);
    ...
    document.getElementById("tipoEntrada").value = tipoEntrada;

    // Función para obtener canciones seleccionadas desde la base de datos
    function obtenerCancionesSeleccionadas() {
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    var cancionesSeleccionadas = JSON.parse(xhr.responseText);
                    if (cancionesSeleccionadas.length > 0) {
                        carritoHTML += '<h4>Canciones seleccionadas:</h4>';
                        cancionesSeleccionadas.forEach(function(cancion) {
                            carritoHTML += 'Canción: ' + cancion.name + ' - Artista: ' + cancion.artista_cancion +
                                '<br>';
                        });
                    } else {
                        carritoHTML += 'No se han seleccionado canciones.<br>';
                    }
                    document.getElementById('carritoInfo').innerHTML = carritoHTML; // Actualizar el HTML
                } else {
                    console.error('Error al obtener las canciones seleccionadas:', xhr.status);
                }
            }
        };

        xhr.open('GET', '/cliente/cancionesSeleccionadas', true);
        xhr.send();
    }

    // Función para obtener bonificaciones canjeadas desde la base de datos
    function obtenerBonificacionesCanjeadas() {
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    var bonificacionesCanjeadas = JSON.parse(xhr.responseText);
                    if (bonificacionesCanjeadas.length > 0) {
                        carritoHTML += '<h4>Bonificaciones canjeadas:</h4>';
                        bonificacionesCanjeadas.forEach(function(bonificacion) {
                            carritoHTML += 'Bonificación: ' + bonificacion.name + '<br>';
                        });
                    } else {
                        carritoHTML += 'No se han canjeado bonificaciones.<br>';
                    }
                    document.getElementById('carritoInfo').innerHTML = carritoHTML; // Actualizar el HTML
                } else {
                    console.error('Error al obtener las bonificaciones canjeadas:', xhr.status);
                }
            }
        };

        xhr.open('GET', '/cliente/bonificacionesCanjeadas', true);
        xhr.send();
    }


    carritoHTML += 'Total de entradas del carrito: <b>' + totalEntradas + '</b><br>';

    // Mostrar la información del carrito dentro del elemento address
    document.getElementById('carritoInfo').innerHTML = carritoHTML;
</script>
